  ...
  private val ICMAImplement = new DayCountConvention with ReferencePeriod {
    ...
    override def fractionOfYear(date1: LocalDate, date2: LocalDate, date3: LocalDate, freq: Frequency): Double = {
      ...
      (date1, date2) match {
        ...
        case (date1, date2) if (date1 > date2) => -fractionOfYear(date2, date1, date3, freq)
    ...
    override def fractionOfYear(date1: LocalDate, date2: LocalDate, date3: LocalDate, freq: Frequency): Double = {
      (date1, date2) match {
        ...
        case (date1, date2) =>
          ...
          val fractionOfYearStart = dayCount(date1, new LocalDate(date1.getYear + 1, JANUARY, 1)) / date1Base
          val fractionOfYearEnd = dayCount(new LocalDate(date2.getYear, JANUARY, 1), date2) / date2Base
