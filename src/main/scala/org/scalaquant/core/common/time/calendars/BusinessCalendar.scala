...
trait BusinessCalendar {
  ...
  def advance(date: LocalDate, n: Int, unit: TimeUnit.TimeUnit,
    ...
    if (n == 0) {
      ...
      unit match {
        case TimeUnit.Days => adjust(date.plusDays(n), convention)
        case TimeUnit.Weeks => adjust(date.plusWeeks(n), convention)
        case TimeUnit.Months | TimeUnit.Years =>
          val advancedDate = if (unit == TimeUnit.Months) date.plusMonths(n) else date.plusYears(n)
...
object BusinessCalendar{
  ...
  type DateVerfication = LocalDate => Boolean
  trait Market

  def dayRanges(from: LocalDate, to: LocalDate,
