...
package object payoffs {
  ...
  final case class FloatingTypePayoff(optionType: Option.Type) extends TypedPayoff("FloatingType", optionType)

  final case class PlainVanillaPayoff(optionType: Option.Type, strike: Double) extends StrikedPayoff("Vanilla", optionType) {
    def apply(price: Double): Double = payoff(max(price - strike, 0.0), max(strike - price, 0.0))
  }

  final case class PercentageStrikePayoff(optionType: Option.Type, private val moneyness: Double) extends StrikedPayoff(moneyness, "PercentageStrike", optionType){
    def apply(price: Double): Double = payoff(max(1.0 - moneyness, 0.0), max(moneyness - 1.0, 0.0))
  }

  final case class AssetOrNothingPayoff(optionType: Option.Type, strike: Double) extends StrikedPayoff("AssetOrNothing", optionType){
    def apply(price: Double): Double = payoff(if (price - strike > 0.0) price else 0.0, if (strike - price > 0.0) price else 0.0)
  }

  final case class CashOrNothingPayoff(optionType: Option.Type, strike: Double, cashPayoff: Double) extends StrikedPayoff("CashOrNothing", optionType){
  ...
  final case class GapPayoff(optionType: Option.Type, strike: Double, secondStrike: Double) extends StrikedPayoff("Gap", optionType){
  ...
  final case class SuperFundPayoff(strike: Double, secondStrike: Double) extends StrikedPayoff("SuperFund", Option.Call) {
  ...
  final case class SuperSharePayoff(strike: Double, secondStrike: Double, cashPayoff: Double) extends StrikedPayoff("SuperShare", Option.Call){
    require(secondStrike>strike, s"second strike ($secondStrike) must be higher than first strike ($strike)")
